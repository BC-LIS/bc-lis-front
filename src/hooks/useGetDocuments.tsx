...
const useFilteredDocuments = () => {
  ...
  const fetchFilteredDocuments = async (
    endpoint: string,
    filterParams: DocumentsFilters
  ) => {
    try {
      const response = await fetch(
        `${endpoint}?${new URLSearchParams(filterParams).toString()}`,
        {
          method: "GET",
          headers: {
      ...
      try {
        ...
        if (!response.ok) {
          toast({
            title: "Error ❌",
            description:
              "No se ha podido abrir el documento, inténtalo nuevamente.",
          });
          return;
        }
...
export { useFilteredDocuments, useGetDocumentsID, useDownloadDocuments };
